CREATE DATABASE IF NOT EXISTS hw4_base;
USE hw4_base;
...
--    Выведем всех котиков по магазинам по id (условие соединения shops.id = cats.shops_id) 
...
-- Выведем магазин, в котором продается кот “Мурзик” (попробуйте выполнить 2 способами)
-- подзапрос:
...
WHERE c.name NOT IN ('Murzik', 'Zuza') OR c.name IS NULL;
...
VALUES 
    ...
    -- Выведем название и цену для всех анализов, которые продавались 5 февраля 2020 и всю следующую неделю.

SELECT an_name AS 'Название', an_price AS 'Цена'
FROM Analysis
WHERE an_id IN (
    SELECT ord_an
    FROM Orders
    WHERE ord_datetime >= '2020-02-05 00:00:00' AND ord_datetime < '2020-02-12 00:00:00'
);
